			...
			divConteiner.appendChild(br);
			   ...
			   anwers.forEach( (el1) =>{
				 ...
				 if(el1['category'] == el['category']){
					 ...
					 label.innerHTML = `<textarea class='${el1['id']}' style="visibility:hidden; position: absolute;">${el1['answer'] }</textarea><button class='btn btn-outline-dark'  id='fun_${el1['id']}' onclick="copy_answer( ${el1['id']}, '${el1['behavior']}' )" data-bs-toggle="tooltip" title="${el1['answer'] }">  ${el1['title']} </button>` 
