...
function fun_troca_plano (){
    ...
    descricao.value = `Cliente solicita informações sobre troca de plano. Repassados os planos em catálogo para cliente. Orientado (a) a enviar um e-mail com a solicitação da troca para novoplano@fasternet.com.br ou solicitar presencialmente no escritório local.  Cliente ciente que para o mês posterior à solicitação, serão geradas duas faturas com os valores proporcionais aos períodos em que cada plano foi utilizado.`
			...
			divConteiner.appendChild(br);
			   ...
			   anwers.forEach( (el1) =>{
				 ...
				 if(el1['category'] == el['category']){
					 ...
					 label.innerHTML = `<textarea class='${el1['id']}' style="visibility:hidden; position: absolute;">${el1['answer'] }</textarea><button class='btn btn-outline-dark'  id='fun_${el1['id']}' onclick="copy_answer( ${el1['id']}, '${el1['behavior']}' )" data-toggle="tooltip" data-original-title="${el1['answer'] }">  ${el1['title']} </button>` 
